  ...
  <div class="houseList">
    <article>
      ...
      <main class="main">
        ...
        <el-card shadow="hover">
          <div slot="header" class="clearfix">
            <i class="el-icon-s-home" style="color: #67c23a" />
            <span>选定房屋</span>
            <el-popover
              placement="left"
              title="示例说明"
...
export default {
  ...
  activated() {
    ...
    if (this.$route.query.contractId === undefined) {
      if (Cookies.get("houseList") !== undefined) {
        this.selectedHouse = JSON.parse(Cookies.get("houseList"));
      } else {
        this.defaultCheckHouse();
      }
    } else {
      this.getCheckHouse();
    }
  ...
  methods: {
    ...
    saveData() {
      ...
      Cookies.set("houseList", JSON.stringify(this.selectedHouse));
      Cookies.set("houseLists", JSON.stringify(newArr));
      this.$router.go(-1);
    },
    // 取消
    cancel() {
